...
urlpatterns = [
    ...
    path("password-reset/", RedirectView.as_view(url="/ingresar/", permanent=True)),
    path("password-reset/done/", RedirectView.as_view(url="/ingresar/", permanent=True)),
    path(
        "password-reset/<uidb64>/<token>/",
        RedirectView.as_view(url="/ingresar/", permanent=True),
    ),
    path("password-reset/complete/", RedirectView.as_view(url="/ingresar/", permanent=True)),
